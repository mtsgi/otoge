...
namespace NfcPcSc
{
    public class NfcMain : MonoBehaviour
    {
        ...
        private void ReaderStateCheck(int index)
        {
            ...
            {
                if ((readerStateArray[index].dwEventState & NfcConstant.SCARD_STATE_PRESENT) == NfcConstant.SCARD_STATE_PRESENT)
                {
                    if (isBeginTouch)
                    {
                        Debug.Log(index+" に タッチし始め");
                        readCard(context, readerStateArray[index].szReader);
                        SendCommand(context, readerStateArray[index].szReader);
                        isTouchingCard = true;
                        isBeginTouch = false;
                    }
